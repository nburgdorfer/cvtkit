...
def get_epipolar_inds(x0, y0, x1, y1):
    dx = x1-x0
    dy = y1-y0
    ...
    max_disp = int((x1-x0).max().item())

    xy = []
    for _ in range(max_disp):
        xy.append(torch.stack([x,y],dim=-1))
...
def epipolar_patch_retrieval(imgs, intrinsics, extrinsics, patch_size):
    ...
    x_flat = torch.arange((half_patch_size),width+(patch_size))[::patch_size].to(imgs)
    y_flat = torch.arange((half_patch_size),height+(patch_size))[::patch_size].to(imgs)
    ...
    for i in range(1,imgs.shape[1]):
        ...
        x_inds, y_inds = get_epipolar_inds(x0,y0,x1,y1)
