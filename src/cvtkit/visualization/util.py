...
def mouse1_callback(event, x, y, flags, param):
    if event == cv2.EVENT_MOUSEMOVE:
        ...
        if CENTERED:
            i2ray[2] = (
                i2ray[2] - i2ray[0] * img2.shape[1] / 2 - i2ray[1] * img2.shape[0] / 2
            )
        # print(i2ray)
        i2pt1_x = 0
...
def scale_img(img, scale):
    img_scaled = transform.resize(
        img, [img.shape[0] * scale, img.shape[1] * scale], mode="constant"
    )
...
def plot_coverage(data, output, batch_ind, vis_path):
    ...
    for l in range(levels):
        ...
        coverage = torch.clip(
            torch.where(diff <= min_interval, 1, 0).sum(dim=1, keepdim=True), 0, 1
        )
        uncovered = torch.clip(
            torch.where(coverage <= 0, 1, 0).sum(dim=1, keepdim=True), 0, 1
        )
        valid_targets = torch.where(target_depth > 0, 1, 0)
        uncovered *= valid_targets
        cov_percent = coverage.sum() / (valid_targets.sum() + 1e-10)
        coverage = coverage.reshape(h, w, 1).repeat(1, 1, 3) * (
            torch.tensor([[[0, 255, 0]]]).to(coverage).repeat(h, w, 1)
        )
        uncovered = uncovered.reshape(h, w, 1).repeat(1, 1, 3) * (
            torch.tensor([[[255, 0, 0]]]).to(uncovered).repeat(h, w, 1)
        )

        # plot
        total_coverage = torch.movedim(coverage + uncovered, (0, 1, 2), (1, 2, 0))
        total_coverage = torch.movedim(
            tvf.resize(
                total_coverage, [H, W], interpolation=tvt.InterpolationMode.NEAREST
            ),
            (0, 1, 2),
            (2, 0, 1),
        )
...
def visualize_relative_variance(
    target_cost,
    cost_volume,
    hypos,
    intervals,
    ...
    var_diff = torch.div(tcv, cv + 1e-10)[
...
def visualize_point_variance(
    ...
    var_mismatch = var_mismatch[
        torch.where(var_mismatch >= (mismatch_mean - (3 * mismatch_sigma)))[0]
    ]
    var_mismatch = var_mismatch[
        torch.where(var_mismatch <= (mismatch_mean + (3 * mismatch_sigma)))[0]
    ]
    ...
    with open(out_file, "w") as of:
        ...
        for i in range(num_rays):
            for j in range(num_points):
                of.write(
                    f"{rays[i,j,0]:0.3f} {rays[i,j,1]:0.3f} {rays[i,j,2]:0.3f} 0 0 0\n"
                )
...
def laplacian_count(data, output, plot_file=None, use_est_depth=False):
    ...
    if plot_file != None:
        ...
        for i in range(5):
            for j in range(5):
                text = ax.text(
                    i, j, f"{(M[j, i]*100):0.2f}%", ha="center", va="center", color="w"
                )
